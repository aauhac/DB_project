...
class BackpackRepository(BaseRepository):
    """support_item(backpack) CRUD."""

    def find_all(self) -> list[dict[str, Any]]:
        return self.fetch_all("SELECT * FROM support_item WHERE item_type = 'backpack' ORDER BY id")

    def find_by_id(self, item_id: int) -> dict[str, Any] | None:
        return self.fetch_one(
            "SELECT * FROM support_item WHERE id = ? AND item_type = 'backpack'",
            (item_id,),
        )
    ...
    def update(self, item_id: int, data: dict[str, Any]) -> None:
        self.execute(
            ...
            WHERE id = ? AND item_type = 'backpack'
            """,
            (data["name"], data["capacity"], data.get("weight"), data.get("description"), item_id),
        )

    def delete(self, item_id: int) -> None:
        self.execute("DELETE FROM support_item WHERE id = ? AND item_type = 'backpack'", (item_id,))
